<?php
/**
 * public/ajax/service.php - Endpoint AJAX para Services
 */
...
require_once ROOT . '/app/controllers/ServiceController.php';
...
try {
    ...
    switch ($a) {
        ...
        case 'ticket_count':
            if ($_SERVER['REQUEST_METHOD'] !== 'GET') { http_response_code(405); break; }
            $id = $_GET['id'] ?? null;
            echo json_encode($id ? $c->getTicketCount($id) : ['success' => false, 'message' => 'ID requerido']);
            break;
        case 'statistics':
    ...
    error_log("SERVICE AJAX: " . $e->getMessage());
