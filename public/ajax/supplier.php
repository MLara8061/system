<?php
/**
 * public/ajax/supplier.php - Endpoint AJAX para Suppliers
 */
...
try {
    ...
    switch ($a) {
        ...
        case 'by_country':
            ...
            echo json_encode($country ? $c->getByCountry($country) : ['success' => false, 'message' => 'País requerido']);
            break;
        case 'equipment_count':
            if ($_SERVER['REQUEST_METHOD'] !== 'GET') { http_response_code(405); break; }
            $id = $_GET['id'] ?? null;
            echo json_encode($id ? $c->getEquipmentCount($id) : ['success' => false, 'message' => 'ID requerido']);
            break;
        default:
    ...
    error_log("SUPPLIER AJAX: " . $e->getMessage());
