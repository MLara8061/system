...
if (empty($token) || !preg_match('/^[a-f0-9]{32}$/', $token)) {
    ...
    if ($qry && $qry->num_rows > 0) {
        ...
        // Obtener historial publico (solo cambios de estado, sin notas internas)
        $tid = (int)$ticket['id'];
        ...
        if ($h) {
            while ($row = $h->fetch_assoc()) $history[] = $row;
        }
        // Obtener comentarios publicos (no internos)
        $comments = [];
        ...
        if ($cq) {
            while ($row = $cq->fetch_assoc()) $comments[] = $row;
        }
    } else {
...
<?php else: ?>
    ...
    <div class="p-4">
        ...
        <div class="status-progress">
            <div class="progress-step <?php echo $st >= 0 ? 'done' : ''; ?>">
        ...
        <div class="timeline">
            ...
            <?php 
            // Merge histories and comments chronologically
            $merged = [];
            foreach ($history as $h) {
                $merged[] = ['type' => 'status', 'date' => $h['created_at'], 'data' => $h];
            }
            foreach ($comments as $c) {
                $merged[] = ['type' => 'comment', 'date' => $c['date_created'], 'data' => $c];
            }
                ...
                <div class="tl-item">
                    ...
                    <div class="tl-text">
                        <span class="comment-author <?php echo $isReporter ? 'reporter' : 'support'; ?>">
                            <?php if ($isReporter): ?><i class="fas fa-user" style="font-size:0.75rem;"></i><?php else: ?><i class="fas fa-headset" style="font-size:0.75rem;"></i><?php endif; ?>
        ...
        <!-- Formulario de respuesta del reportante -->
        <?php if ($ticket && (int)$ticket['status'] < 3): ?>
...
$(function() {
    $('#public-comment-btn').on('click', function() {
        ...
        if (!msg) {
            showAlert('warning', 'Por favor escribe un mensaje antes de enviar.');
            return;
        }
