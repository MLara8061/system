				...
				<div class="row">
					...
					<div class="col-md-6">
						...
						<div class="form-group">
							...
							<small><i><?php echo isset($id) ? "Deja este campo en blanco sino quieres cambiar la contrase̱a" : '' ?></i></small>
						</div>
						<div class="form-group">
...
<script>
	$('[name="password"],[name="cpass"]').keyup(function() {
		...
		if (cpass == '' || pass == '') {
			...
			if (cpass == pass) {
				$('#pass_match').attr('data-status', '1').html('<i class="text-success">Contrase̱as Coinciden</i>')
			} else {
				$('#pass_match').attr('data-status', '2').html('<i class="text-danger">Contrase̱as no Coinciden</i>')
			}
		}
	})
	...
	$('#manage_staff').submit(function(e) {
		...
		$.ajax({
			url: 'public/ajax/action.php?action=save_staff',
