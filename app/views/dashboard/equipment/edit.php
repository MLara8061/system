            ...
            <div class="row g-0">
                ...
                <div class="col-lg-7 p-5">
                    <form id="manage_equipment" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" name="serie" id="serie" class="form-control alfanumerico" 
                                           required value="<?= htmlspecialchars($eq['serie'] ?? '') ?>">
                        ...
                        <?= CustomFieldRenderer::render('equipment', (int)$equipment_id) ?>
                    </form>
                </div>
            </div>
...
<script>
    ...
    $(function(){
        ...
        // Sincronizar disciplina con categoría (alineado con Nuevo Equipo)
        function syncDisciplineFromCategory(){
            var desc = $('#equipment_category_id option:selected').attr('data-desc') || '';
            $('#discipline').val(desc);
        }
        $('#equipment_category_id').on('change', function(){
            syncDisciplineFromCategory();
        ...
        $('#department_id').on('select2:select change', function(){
            ...
            if(department_id){
                $.ajax({
                    url: 'public/ajax/action.php?action=get_locations_by_department',
                    method: 'POST',
        ...
        $('#location_id').on('select2:select change', function(){
            ...
            if(location_id){
                $.ajax({
                    url: 'public/ajax/action.php?action=get_job_positions_by_location',
                    method: 'POST',
    ...
    (function(){
        var timer = null;
        var serieOk = true;
        var equipId = <?= (int)$equipment_id ?>;
        var originalSerie = <?= json_encode($eq['serie'] ?? '') ?>;
        var $input = $('#serie');
        ...
        function checkSerie() {
            ...
            $.get('public/ajax/action.php', { action: 'check_serie', serie: val, exclude_id: equipId }, function(r) {
                if (r.available) {
